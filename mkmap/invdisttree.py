...
# error analysis, |f(x) - idw(x)| ? todo: regular grid, nnear ndim+1, 2*ndim
    ...
    def __call__(self, q, nnear=6, eps=0, p=1, weights=None,method='new'):
...
# new, optimized method:
        if method == 'new':
            ...
            w1 = 1./np.maximum(self.distances,1e-10)**p
            if weights is not None:
                w1 *= weights[self.ix]
            w1 = np.divide(w1,np.sum(w1,axis=1)[:,np.newaxis])
            wz1 = np.einsum('ij,ij->i',w1,self.z[self.ix])
            if self.stat:
                self.wn = len(wz1)
                self.wsum = sum(w1)
...
if __name__ == "__main__":
    ...
    N = 100000
    Ndim = 2
    Nask = 100000  # N Nask 1e5: 24 sec 2d, 27 sec 3d on mac g4 ppc
    ...
    print "\nInvdisttree:  N %d  Ndim %d  Nask %d  Nnear %d  leafsize %d  eps %.2g  p %.2g method %s" % (
        N, Ndim, Nask, Nnear, leafsize, eps, p, method)
    ...
    z = terrain(known)
    #z = np.sin(np.sum(known,axis=1))
    ask = np.random.uniform(size=(Nask, Ndim))

#.........................................................................
    start = timer()
    invdisttree = Invdisttree(known, z, leafsize=leafsize, stat=1)
    print 'time for building tree: ',timer()-start

    start = timer()
    interpol = invdisttree(ask, nnear=Nnear, eps=eps, p=p, method=method)
    print 'time for interpolation: ',timer()-start
    ...
    print invdisttree((0.888,0.10),nnear=Nnear, eps=eps,p=p)
    print invdisttree((0.889,0.11),nnear=Nnear, eps=eps,p=p)
    print invdisttree((0.887,0.12),nnear=Nnear, eps=eps,p=p)
